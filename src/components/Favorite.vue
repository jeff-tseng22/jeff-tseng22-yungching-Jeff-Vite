<template>
  <div>
    <h1 class="favorite_title">我的最愛列表</h1>
    <div class="page_link">
    ...
    <table>
      ...
      <tbody>
        ...
        >
          ...
          <td>
            <button @click="startEdit(market.id)">編輯</button>
          </td>
          <td>
            <img
    ...
    <div v-if="editId !== null" class="edit_area">
      <h3>編輯資料</h3>
  ...
  export default {
    ...
    setup() {
      ...
      const startEdit = (id) => {
        ...
        const market = filteredData.value.find((item) => item.id === id);
        if (market) {
          // 更新編輯中的資料 ID 和表單內容
          editId.value = id;
...
<style scoped lang="scss">
  ...
  input[type="text"] {
    ...
    padding: 3px;
    font-size: 1rem;
    border: 1px solid rgba(240, 203, 203, 0.931);
